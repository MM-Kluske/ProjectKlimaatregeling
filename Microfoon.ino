...
loop()
{
  int32_t value;
  char dbOutput[256];
  const float Vref = 1024; // Referentiewaarde voor dB-berekening
  //char withScale[256];
  ...
  if (rx_buf_flag) {
    for (int x = 0; x < SLC_BUF_LEN; x++) {
      if (i2s_slc_buf_pntr[rx_buf_idx][x] > 0) {
        ...
        // Berekening van dB-waarde
        float dB = 20.0 * log10(abs((float)value) / Vref);
        
        // Formatteer de uitvoer
        sprintf(dbOutput, "%.2f dB", dB);
        Serial.println(dbOutput);
